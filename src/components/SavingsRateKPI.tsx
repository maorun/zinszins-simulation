...
function SavingsRateDisplay({ savingsRate, colorClass, category, trend }: {
  savingsRate: number;
  colorClass: string;
  category: 'excellent' | 'good' | 'average' | 'low';
  trend: { icon: typeof TrendingUp; text: string; class: string } | null;
}) {
...
function SavingsRateMetrics({ monthlySavings, monthlyIncome }: { monthlySavings: number; monthlyIncome: number }) {
  return (
...
function useSavingsRateTrend(savingsRate: number, previousSavingsRate: number | undefined, showComparison: boolean) {
  return useMemo(() => {
...
export function SavingsRateKPI({
  ...
  const trend = useSavingsRateTrend(savingsRate, previousSavingsRate, showComparison);
  ...
  return (
    <Card id={cardId} className="w-full">
      ...
      <CardContent className="space-y-4">
        <SavingsRateDisplay savingsRate={savingsRate} colorClass={colorClass} category={category} trend={trend} />
        <Progress value={savingsRate} className="h-2" />
        <SavingsRateMetrics monthlySavings={monthlySavings} monthlyIncome={monthlyIncome} />
