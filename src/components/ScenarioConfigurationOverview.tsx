  ...
  return (
    <div>
      <span className="text-blue-700 font-medium">
        {useGlossary && glossaryTerm ? <GlossaryTerm term={glossaryTerm}>{label}</GlossaryTerm> : label}:
      </span>
...
export function ScenarioConfigurationOverview({ scenario }: ScenarioConfigurationOverviewProps) {
  ...
  return (
    <div className="bg-blue-50 border border-blue-200 p-4 rounded-lg">
      <h3 className="font-semibold text-blue-900 mb-3 flex items-center gap-2">📊 Szenario-Konfiguration</h3>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
        ...
        {scenario.config.initialInvestment && (
          <ConfigField label="Startkapital" value={`${scenario.config.initialInvestment.toLocaleString('de-DE')}€`} />
        )}
