...
export const CollapsibleCardHeader = React.forwardRef<HTMLDivElement, CollapsibleCardHeaderProps>(
  ({ children, titleClassName, iconClassName, simplifiedPadding, ...props }, ref) => {
    ...
    const defaultIconClasses = simplifiedPadding ? 'h-5 w-5' : 'h-5 w-5 sm:h-4 sm:w-4'

    return (
      <CardHeader nestingLevel={nestingLevel} {...props} ref={ref}>
        <CollapsibleTrigger asChild>
          <div
            className={`flex items-center justify-between w-full cursor-pointer hover:bg-gray-50 rounded-md transition-colors group ${paddingClasses}`}
          >
            <CardTitle className={titleClassName}>{children}</CardTitle>
            <ChevronDown
              className={`${iconClassName || defaultIconClasses} text-gray-500 transition-transform duration-200 group-data-[state=open]:rotate-180`}
            />
          </div>
        </CollapsibleTrigger>
      </CardHeader>
...
export const CollapsibleCardContent = React.forwardRef<HTMLDivElement, CardContentProps>(
  ({ children, ...props }, ref) => {
    const nestingLevel = useNestingLevel()
      ...
      <CollapsibleContent>
        <CardContent nestingLevel={nestingLevel} {...props} ref={ref}>
          <NestingProvider level={nestingLevel}>
            <div className="form-grid space-y-4">{children}</div>
          </NestingProvider>
        </CardContent>
      </CollapsibleContent>
    )
