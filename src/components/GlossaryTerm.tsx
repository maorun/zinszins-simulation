 ...
 */
export function GlossaryTerm({ term, children, showIcon = false, className = '' }: GlossaryTermProps) {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent className="max-w-sm p-4 bg-white border-2 border-blue-200 shadow-lg" side="top" sideOffset={8}>
        <div className="space-y-2">
          <div className="font-semibold text-blue-900 text-base border-b border-blue-200 pb-2">
            {glossaryEntry.term}
          </div>
          <div className="text-sm text-gray-700 font-medium">{glossaryEntry.shortDefinition}</div>
          <div className="text-sm text-gray-600 leading-relaxed">{glossaryEntry.detailedExplanation}</div>
