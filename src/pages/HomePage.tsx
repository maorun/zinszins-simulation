...
import { useScenarioHandler } from '../hooks/useScenarioHandler'
import { useReturnConfig } from '../hooks/useReturnConfig'
import { calculatePhaseDateRanges } from '../utils/phase-date-ranges'
import { SavingsPhaseOverview } from '../components/SavingsPhaseOverview'
import { WithdrawalPhaseOverview } from '../components/WithdrawalPhaseOverview'
...
function EnhancedOverview() {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
      ...
      <SavingsPhaseOverview
        savingsStartYear={savingsStartYear}
        savingsEndYear={savingsEndYear}
        startkapital={enhancedSummary.startkapital}
        endkapital={enhancedSummary.endkapital}
        zinsen={enhancedSummary.zinsen}
        renditeAnsparphase={enhancedSummary.renditeAnsparphase}
      />
      {enhancedSummary.endkapitalEntspharphase !== undefined && (
        <WithdrawalPhaseOverview
          withdrawalStartYear={startEnd[0] + 1}
          withdrawalEndYear={withdrawalEndYear}
          endkapitalEntspharphase={enhancedSummary.endkapitalEntspharphase}
          monatlicheAuszahlung={enhancedSummary.monatlicheAuszahlung}
          isSegmentedWithdrawal={enhancedSummary.isSegmentedWithdrawal}
          withdrawalSegments={enhancedSummary.withdrawalSegments}
        />
      )}
    </div>
  )
...
const HomePageContent = () => {
  ...
  const { handleApplyScenario } = useScenarioHandler({
    setStartEnd,
    setReturnMode,
    setRendite,
    setAverageReturn,
  ...
  const returnConfig = useReturnConfig({
    returnMode,
    rendite,
    averageReturn,
    standardDeviation,
