...
describe('dynamic-savings-rate', () => {
  ...
  describe('calculateIncomeDevelopmentAdjustment', () => {
    it('should return 0 when income development is disabled', () => {
      const adjustment = calculateIncomeDevelopmentAdjustment(
        1990,
        2024,
        2025,
        2000,
        { enabled: false, savingsRateOfIncrease: 0.5, salaryIncreases: [] },
      )
    ...
    it('should calculate cumulative adjustments correctly', () => {
      ...
      const adj1Year = calculateIncomeDevelopmentAdjustment(
        1990,
        2024,
        2025,
        2000,
      ...
      const adj2Years = calculateIncomeDevelopmentAdjustment(
        1990,
        2024,
        2026,
        2000,
  ...
  describe('validateDynamicSavingsConfig', () => {
    ...
    it('should detect overlapping life phases', () => {
      ...
      const errors = validateDynamicSavingsConfig(config)
      expect(errors.some((e) => e.field === 'lifePhases' && e.message.includes('überschneidet'))).toBe(
        true,
      )
    })
    ...
    it('should validate life phase age ranges', () => {
      ...
      expect(
        errors.some(
          (e) => e.field === 'lifePhases' && e.message.includes('Startalter muss kleiner'),
        ),
      ).toBe(true)
    })
    ...
    it('should validate negative multipliers', () => {
      const config: DynamicSavingsRateConfig = {
        ...
        birthYear: 1990,
        lifePhases: [
          { phase: 'berufsstart', startAge: 25, endAge: 30, savingsRateMultiplier: -0.5 },
        ],
      }
      ...
      expect(
        errors.some(
          (e) => e.field === 'lifePhases' && e.message.includes('darf nicht negativ sein'),
        ),
      ).toBe(true)
    })
    ...
    it('should validate income development savings rate', () => {
      ...
      const errors = validateDynamicSavingsConfig(config)
      expect(
        errors.some((e) => e.field === 'incomeDevelopment.savingsRateOfIncrease'),
      ).toBe(true)
    })
    ...
    it('should validate negative salary increases', () => {
      ...
      const errors = validateDynamicSavingsConfig(config)
      expect(
        errors.some((e) => e.field === 'incomeDevelopment.salaryIncreases'),
      ).toBe(true)
    })
