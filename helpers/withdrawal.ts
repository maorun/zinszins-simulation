...
> = {
  ...
  random: (allYears, config) => (config.randomConfig ? generateRandomReturns(allYears, config.randomConfig) : {}),
  variable: (allYears, config) =>
    config.variableConfig ? generateVariableGrowthRates(allYears, config.variableConfig) : {},
  multiasset: (allYears, config) =>
    config.multiAssetConfig ? generateMultiAssetGrowthRates(allYears, config.multiAssetConfig) : {},
}
...
function determineYearsForGrowthRates(
  ...
  const needsPreviousYear =
    strategy === 'dynamisch' || (strategy === 'bucket_strategie' && bucketConfig?.subStrategy === 'dynamisch')

  return needsPreviousYear ? Array.from({ length: endYear - startYear + 2 }, (_, i) => startYear - 1 + i) : years
}
...
function initializeOtherIncomeData(
  ...
  return otherIncomeConfig?.enabled ? calculateOtherIncome(otherIncomeConfig, startYear, endYear) : {}
}
...
function calculateMonthlyWithdrawal(
  ...
  const effectiveWithdrawal =
    withdrawalFrequency === 'monthly' ? calculateMonthlyEffectiveWithdrawal(entnahme, returnRate) : entnahme
...
function applyWithdrawalAdjustments(
  strategy: WithdrawalStrategy,
  baseAmount: number,
  rmdAmount: number,
  year: number,
  ...
  withdrawal += dynamischeAnpassung

  return { withdrawal, inflationAnpassung, dynamischeAnpassung, vorjahresRendite }
}

type AdjustedWithdrawalParams = {
  strategy: WithdrawalStrategy
  baseWithdrawalAmount: number
  capitalAtStartOfYear: number
  year: number
...
function calculateAdjustedWithdrawal(params: AdjustedWithdrawalParams): AdjustedWithdrawalResult {
  ...
  const { optimizedWithdrawal, steueroptimierungAnpassung } = applyTaxOptimization({
    ...
    steueroptimierteEntnahmeConfig: params.steueroptimierteEntnahmeConfig,
    getFreibetragForYear: params.getFreibetragForYear,
    year: params.year,
 ...
 */
function buildStatutoryPensionField(year: number, statutoryPensionData: StatutoryPensionResult) {
 ...
 */
function buildOtherIncomeField(year: number, otherIncomeData: OtherIncomeResult) {
...
/**
 * Calculate initial capital and validate if withdrawal should proceed
 */
function calculateInitialCapitalAndValidate(
  mutableLayers: MutableLayer[],
  cashCushion: number,
): { capitalAtStartOfYear: number; shouldContinue: boolean; updatedCashCushion: number } {
  const capitalAtStartOfYear = mutableLayers.reduce((sum: number, l: MutableLayer) => sum + l.currentValue, 0)
  if (capitalAtStartOfYear <= 0) {
    return { capitalAtStartOfYear: 0, shouldContinue: false, updatedCashCushion: cashCushion }
  }
  return { capitalAtStartOfYear, shouldContinue: true, updatedCashCushion: cashCushion }
}
 ...
 */
function getYearRealizedGainsTax(params: YearlyTaxesParams, totalRealizedGain: number, yearlyFreibetrag: number) {
...
function calculateYearlyTaxes(params: YearlyTaxesParams): YearlyTaxesResult {
  const capitalAtStartOfYear = params.mutableLayers.reduce((sum: number, l: MutableLayer) => sum + l.currentValue, 0)

  const { totalPotentialVorabTax, vorabCalculations, yearlyFreibetrag, basiszins } = getYearVorabpauschaleData(params)

  const totalRealizedGainThisYear = processLayerWithdrawal(
    params.mutableLayers,
    params.effectiveWithdrawal,
    params.strategy,
    params.bucketUsed,
  )
...
/**
 * Process bucket strategy refill and calculate final capital
 */
function processBucketRefillAndFinalCapital(params: {
  strategy: WithdrawalStrategy
  bucketConfig: BucketStrategyConfig | undefined
  returnRate: number
  capitalAtStartOfYear: number
  ...
  const capitalAtEndOfYear = params.mutableLayers.reduce((sum: number, l: MutableLayer) => sum + l.currentValue, 0)
  ...
  const refillResult = processCashCushionRefill(
    ...
    params.bucketUsed,
    params.entnahme,
    params.currentCashCushion,
    params.mutableLayers,
  )

  const finalCapitalAtEndOfYear = params.mutableLayers.reduce((sum: number, l: MutableLayer) => sum + l.currentValue, 0)

  return {
    refillAmount: refillResult.refillAmount,
    finalCashCushion: refillResult.updatedCashCushion,
    finalCapitalAtEndOfYear,
  }
}

/**
 * Extract and prepare parameters for withdrawal processing
 */
type ProcessYearlyWithdrawalParams = {
  year: number
  startYear: number
  mutableLayers: MutableLayer[]
  yearlyGrowthRates: Record<number, number>
...
function buildFinalYearResult(params: BuildFinalYearResultParams): WithdrawalResultElement {
  return buildYearlyResult({
    year: params.year,
    capitalAtStartOfYear: params.capitalAtStartOfYear,
    entnahme: params.withdrawalData.entnahme,
    finalCapitalAtEndOfYear: params.refillData.finalCapitalAtEndOfYear,
    totalTaxForYear: params.totalTaxForYear,
    freibetragUsedOnGains: params.taxResults.freibetragUsedOnGains,
    freibetragUsedOnVorab: params.taxResults.freibetragUsedOnVorab,
    monthlyWithdrawalAmount: params.withdrawalData.monthlyWithdrawalAmount,
    inflationConfig: params.inflationConfig,
    inflationAnpassung: params.withdrawalData.inflationAnpassung,
    enableGrundfreibetrag: params.enableGrundfreibetrag,
    einkommensteuer: params.taxResults.einkommensteuer,
    genutzterGrundfreibetrag: params.taxResults.genutzterGrundfreibetrag,
    taxableIncome: params.taxResults.taxableIncome,
    guenstigerPruefungResultRealizedGains: params.taxResults.guenstigerPruefungResultRealizedGains,
    strategy: params.strategy,
    dynamischeAnpassung: params.withdrawalData.dynamischeAnpassung,
    vorjahresRendite: params.withdrawalData.vorjahresRendite,
    bucketConfig: params.bucketConfig,
    steueroptimierungAnpassung: params.withdrawalData.steueroptimierungAnpassung,
    cashCushionAtStart: params.withdrawalData.cashCushionAtStart,
    cashCushion: params.refillData.finalCashCushion,
    bucketUsed: params.withdrawalData.bucketUsed,
    refillAmount: params.refillData.refillAmount,
    totalVorabpauschale: params.taxResults.totalVorabpauschale,
    vorabpauschaleDetails: params.taxResults.vorabpauschaleDetails,
    statutoryPensionData: params.statutoryPensionData,
    otherIncomeData: params.otherIncomeData,
    healthCareInsuranceData: params.withdrawalData.healthCareInsuranceData,
    healthCareInsuranceConfig: params.healthCareInsuranceConfig,
    coupleHealthCareInsuranceData: params.withdrawalData.coupleHealthCareInsuranceData,
  })
}
 ...
 */
function getOrchestratedWithdrawalData(params: OrchestrateYearlyWithdrawalParams) {
  return processWithdrawalAmounts({
    ...
    steueroptimierteEntnahmeConfig: params.yearParams.steueroptimierteEntnahmeConfig,
    getFreibetragForYear: params.yearParams.getFreibetragForYear,
    taxRate: params.yearParams.taxRate,
    teilfreistellungsquote: params.yearParams.teilfreistellungsquote,
    withdrawalFrequency: params.yearParams.withdrawalFrequency,
    monthlyConfig: params.yearParams.monthlyConfig,
    cashCushion: params.yearParams.cashCushion,
...
function getOrchestratedTaxResults(
  ...
  return calculateYearlyTaxes({
    ...
    year: params.yearParams.year,
    basiszinsConfiguration: params.yearParams.basiszinsConfiguration,
    taxRate: params.yearParams.taxRate,
    teilfreistellungsquote: params.yearParams.teilfreistellungsquote,
    freibetragPerYear: params.yearParams.freibetragPerYear,
    effectiveWithdrawal: withdrawalData.effectiveWithdrawal,
    strategy: params.yearParams.strategy,
    ...
    enableGrundfreibetrag: params.yearParams.enableGrundfreibetrag,
    entnahme: withdrawalData.entnahme,
    getGrundfreibetragForYear: params.yearParams.getGrundfreibetragForYear,
    statutoryPensionData: params.yearParams.statutoryPensionData,
...
function getOrchestratedRefillData(
  ...
  return processBucketRefillAndFinalCapital({
    ...
    bucketConfig: params.yearParams.bucketConfig,
    returnRate: withdrawalData.returnRate,
    capitalAtStartOfYear: taxResults.capitalAtStartOfYear,
    mutableLayers: params.yearParams.mutableLayers,
    bucketUsed: withdrawalData.bucketUsed,
    entnahme: withdrawalData.entnahme,
 ...
 */
function processYearlyWithdrawal(params: ProcessYearlyWithdrawalParams): {
  ...
  const {
    capitalAtStartOfYear,
    shouldContinue,
    updatedCashCushion: initialCushion,
  } = calculateInitialCapitalAndValidate(params.mutableLayers, params.cashCushion)

  if (!shouldContinue) {
    return { yearResult: null, updatedCashCushion: initialCushion, shouldContinue: false }
  }

  const { withdrawalData, taxResults, refillData, totalTaxForYear } = orchestrateYearlyWithdrawalCalculation({
    capitalAtStartOfYear,
    yearParams: params,
  })

  const yearResult = buildFinalYearResult({
    year: params.year,
    capitalAtStartOfYear,
    withdrawalData,
    taxResults,
  ...
  return { yearResult, updatedCashCushion: refillData.finalCashCushion, shouldContinue: true }
}
 ...
 */
function buildGuenstigerPruefungField(guenstigerPruefungResult: GuenstigerPruefungResultType | undefined | null) {
...
function buildYearlyResult(params: BuildYearlyResultParams): WithdrawalResultElement {
  ...
  return {
    ...
    guenstigerPruefungResultRealizedGains: buildGuenstigerPruefungField(params.guenstigerPruefungResultRealizedGains),
...
function applyPortfolioGrowthAndVorabTax(
  ...
  vorabCalculations.forEach(calc => {
    const taxForLayer =
      totalPotentialVorabTax > 0 ? (calc.potentialTax / totalPotentialVorabTax) * taxOnVorabpauschale : 0
    ...
    calc.layer.currentValue = steuerReduzierenEndkapital ? valueAfterGrowth - taxForLayer : valueAfterGrowth
...
function calculateTotalTaxableIncome(params: {
  ...
  const { entnahme, year, statutoryPensionData, otherIncomeData, healthCareInsuranceData, healthCareInsuranceConfig } =
    params
...
function calculateYearHealthCareInsurance(params: {
  ...
  const { healthCareInsuranceConfig, year, entnahme, statutoryPensionData, birthYear } = params
...
/**
 * Initialize all data sources and configurations for the withdrawal calculation.
 */
function initializeWithdrawalData(params: CalculateWithdrawalParams) {
  const {
    startYear,
    endYear,
    strategy,
  ...
  const baseWithdrawalAmount = calculateBaseWithdrawalAmount({
    ...
    bucketConfig: params.bucketConfig,
    rmdConfig: params.rmdConfig,
    kapitalerhaltConfig: params.kapitalerhaltConfig,
    steueroptimierteEntnahmeConfig: params.steueroptimierteEntnahmeConfig,
  })
...
export function calculateWithdrawal(params: CalculateWithdrawalParams): {
  result: WithdrawalResult
  finalLayers: MutableLayer[]
} {
  const {
    getFreibetragForYear,
    getGrundfreibetragForYear,
    ...
    baseWithdrawalAmount,
    initialCashCushion,
  } = initializeWithdrawalData(params)

  const yearParams: WithdrawalYearParams = {
    ...params,
    withdrawalFrequency: params.withdrawalFrequency || 'yearly',
    taxRate: params.taxRate || 0.26375,
    teilfreistellungsquote: params.teilfreistellungsquote || 0.3,
    steuerReduzierenEndkapital: params.steuerReduzierenEndkapital !== false,
  ...
  const result = processAllWithdrawalYears(params.startYear, params.endYear, initialCashCushion, yearParams)

  const finalLayers = updateFinalLayers(mutableLayers, params.endYear)
...
export function calculateSegmentedWithdrawal(
  ...
  for (const segment of sortedSegments) {
    ...
    const lastYearOfSegment = Object.keys(segmentResultData)
      .map(Number)
      .sort((a, b) => b - a)[0]
...
function calculateBalancedWithdrawal(
  ...
  const taxEfficientAdjustment =
    availableFreibetrag > 0 ? (targetFreibetragUsage - availableFreibetrag * 0.5) / availableFreibetrag : 0
 ...
 */
function generateTestWithdrawalAmounts(baseWithdrawalAmount: number, capitalAtStartOfYear: number): number[] {
  return [
 ...
 */
function areWithdrawalInputsValid(capitalAtStartOfYear: number, baseWithdrawalAmount: number): boolean {
