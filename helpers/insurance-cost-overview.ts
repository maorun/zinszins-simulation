 ...
 */
export type InsuranceCategory = 'health' | 'life' | 'disability' | 'care' | 'other'
 ...
 */
function calculateCoverageForYear(config: TermLifeInsuranceConfig, year: number): number {
 ...
 */
function calculateAverageCostsByCategory(yearlyResults: YearlyInsuranceCosts[]): Record<InsuranceCategory, number> {
 ...
 */
function checkHealthInsuranceCosts(summary: InsuranceCostSummary): InsuranceOptimizationRecommendation | null {
  if (summary.averageCostByCategory.health > 8000) {
    return {
      ...
      message:
        'Bei hohen PKV-Beiträgen im Alter kann ein Tarifwechsel innerhalb der PKV oder die Prüfung von Zusatzbausteinen sinnvoll sein.',
    }
  }
  return null
}
 ...
 */
function checkTermLifeInRetirement(summary: InsuranceCostSummary): InsuranceOptimizationRecommendation | null {
  ...
  if (hasTermLifeInRetirement) {
    return {
      ...
      message:
        'Prüfen Sie, ob die Risikolebensversicherung im Ruhestand noch benötigt wird. Bei ausreichendem Vermögen kann auf Risikoabsicherung verzichtet werden.',
    }
  }
  return null
}
 ...
 */
function checkCareInsurance(summary: InsuranceCostSummary): InsuranceOptimizationRecommendation | null {
  if (summary.averageCostByCategory.care === 0) {
    ...
    if (hasRetirementYears) {
      return {
        ...
        message:
          'Sie haben keine Pflegezusatzversicherung konfiguriert. Bei langer Pflegebedürftigkeit können Kosten von 2.000-4.500 €/Monat entstehen. Die gesetzliche Pflegeversicherung deckt typischerweise nur 50-70% der tatsächlichen Kosten.',
      }
    }
 ...
 */
function checkNoInsurances(summary: InsuranceCostSummary): InsuranceOptimizationRecommendation | null {
  if (summary.insuranceTypes.length === 0) {
    return {
      ...
      message:
        'Sie haben noch keine Versicherungen in der Planung berücksichtigt. Krankenversicherung und ggf. Pflegeversicherung sollten in die Planung einbezogen werden.',
    }
  }
  return null
}
