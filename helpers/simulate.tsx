...
export type SimulationResultElement = {
  ...
  genutzterFreibetrag: number;
    verbleibenderFreibetrag: number;
};
    ...
    if (simulationAnnual === SimulationAnnual.monthly) {
        ...
        if (new Date(element.start).getFullYear() <= year) {
          if (new Date(element.start).getFullYear() === year) {
              ...
              if (new Date(element.start).getMonth() + 1 <= month) {
                if (!element.simulation?.[year]?.startkapital) {
                  ...
                  element.simulation[year] = {
                    ...
                    endkapital:
                      (element.simulation[year]?.endkapital || 0) *
                      (1 + wachstumsrateMonth),
                    zinsen: 0,
                    bezahlteSteuer: 0,
                    genutzterFreibetrag: 0,
                  };
            ...
            let kapital =
              ...
              element.einzahlung +
                (element.type === "einmalzahlung" ? element.gewinn : 0);
            ...
            element.simulation[year]["genutzterFreibetrag"] =
      ...
      for (const element of elements) {
        if (new Date(element.start).getFullYear() <= year) {
          ...
          const kapital =
            element.simulation?.[year]?.startkapital || element.einzahlung;
          ...
          element.simulation[year]["bezahlteSteuer"] =
            vorabPauschaleZinzen.steuer;
          element.simulation[year]["endkapital"] -= vorabPauschaleZinzen.steuer;
          element.simulation[year]["zinsen"] =
            element.simulation[year]["endkapital"] -
            element.simulation[year]["startkapital"];
          element.simulation[year]["genutzterFreibetrag"] =
      ...
      for (let element of elements) {
                    console.log('in', freibetragInYear)
        if (new Date(element.start).getFullYear() <= year) {
                    console.log(year, freibetragInYear)
            element = yearlyCalculation(element, year, wachstumsrate, steuerlast, freibetragInYear);
            freibetragInYear = freibetragInYear - element.simulation[year]["genutzterFreibetrag"]
                    console.log('end', freibetragInYear)
        }
      }
    }
  }
...
export function yearlyCalculation(
  element: SparplanElement,
  year: number,
  wachstumsrate: number,
  steuerlast: number,
  freibetragInYear: number,
) {
  let kapital =
    element.simulation[year - 1]?.endkapital ||
  ...
  const endKapital = zinszinsVorabpauschale(
    kapital,
    kapital * (1 + wachstumsrate),
    basiszinsen[year] || basiszinsen[2023] || 0,
    freibetragInYear,
    steuerlast,
  );
  element.simulation[year] = {
    startkapital: kapital,
    endkapital: kapital * (1 + wachstumsrate) - endKapital.steuer,
    zinsen: kapital * wachstumsrate,
    bezahlteSteuer: endKapital.steuer,
    genutzterFreibetrag: 0,
        verbleibenderFreibetrag: endKapital.verbleibenderFreibetrag,
  };
  return element;
}
